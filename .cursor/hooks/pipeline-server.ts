...
// ─── 配置 ───
const PORT = parseInt(process.env.PIPELINE_PORT ?? "19090");
...
const templatesDir =
  (cliArgs["templates-dir"] as string) ?? BUILTIN_TEMPLATES_DIR;
const PIPELINES_DIR =
  (cliArgs["pipelines-dir"] as string) ?? ".cursor/pipelines";
  ...
  try {
    ...
    for (const [k, v] of Object.entries(data)) {
      if (typeof v === "string" && existsSync(v))
        registeredPipelineDirs.set(k, v);
    }
  } catch {}
}
...
function saveRegisteredDirs() {
  writeFileSync(
    REGISTERED_DIRS_FILE,
    JSON.stringify(Object.fromEntries(registeredPipelineDirs), null, 2),
  );
}
  ...
  try {
    ...
    if (pipelineId)
      entries = entries.filter(
        (e: any) => !e.pipeline_id || e.pipeline_id === pipelineId,
      );
...
function readExplorerConclusion(): string {
  try {
    return readFileSync(EXPLORER_FILE, "utf-8").slice(0, 500);
  } catch {
    return "";
  }
}
...
function extractAgentLabel(
  ...
  if (/frontend.?implement|前端.?实现/i.test(text))
...
function inferStage(
  ...
  const searchText = [taskText ?? "", description ?? "", agentType]
    .join(" ")
    .toLowerCase();
  ...
  if (/\b(backend.?implement|implement.?backend|后端.?实现)/i.test(searchText))
    return "implement";
  if (
    /\b(frontend.?implement|implement.?frontend|前端.?实现)/i.test(searchText)
  )
    return "implement";
  if (/\bimplement/i.test(searchText) && agentType !== "explore")
    return "implement";
  if (
    /\b(code.?review|backend.?review|frontend.?review|代码.?审查)/i.test(
      searchText,
    )
  )
    return "code-review";
  if (/\breview/i.test(searchText) && !/code.?review/i.test(searchText))
    return "review";
  if (/\b(qa.?test|e2e.?test|playwright|功能.?验收)/i.test(searchText))
    return "qa-test";
  if (/\b(integration.?test|集成.?测试|test.?writ)/i.test(searchText))
...
function setStageCompleted(
  ...
  if (s.started_at)
    s.duration_ms =
      new Date(completedAt).getTime() - new Date(s.started_at).getTime();
}
...
function findPipelineForEvent(event: HookEvent): PipelineInstance | null {
  ...
  const byChange = all.filter(
    (p) =>
      p.state.change_name &&
      searchText.includes(p.state.change_name.toLowerCase()),
  );
  ...
  const active = all.filter(
    (p) => p.state.current_stage && p.state.active_agents.length > 0,
  );
...
function handleHookEvent(event: HookEvent): object {
  ...
  switch (event.hook_event_name) {
    case "sessionStart": {
      ...
      for (const dir of [templatesDir, ...getAllPipelineDirs()]) {
        for (const f of listTemplateFiles(dir)) {
          try {
            availableTemplates.add(loadTemplate(join(dir, f)).name);
          } catch {}
        }
      }
      if (availableTemplates.size > 0)
        ctx.push(`[可用模板] ${[...availableTemplates].join(", ")}`);
      // 列出已注册的项目目录
      if (registeredPipelineDirs.size > 0) {
        ctx.push(
          `[已注册项目] ${[...registeredPipelineDirs.entries()].map(([k, v]) => `${k}:${v}`).join(", ")}`,
        );
      }
      ...
      return {
        ...
        env: {
          PIPELINE_SERVER: `http://127.0.0.1:${PORT}`,
          PIPELINE_COUNT: String(all.length),
        },
      };
    }
    ...
    case "subagentStart": {
      ...
      const taskText = (event.task ?? "") as string;
      const descText = (event.description ?? "") as string;
      const agentLabel = extractAgentLabel(
        event.subagent_type ?? "",
        taskText,
        descText,
        inst.template,
    ...
    case "subagentStop": {
      ...
      if (stage) {
        const stillActive = state.active_agents.some(
          (a) => inferStage("", a.task, "", inst.template) === stage,
        );
        if (!stillActive) {
          const existing = state.stages.find((s) => s.name === stage);
    ...
    case "stop": {
      ...
      broadcast("pipeline_checkpoint", {
        pipelines: listPipelines().map((p) => p.state),
      });
      return {};
    }
...
const DASHBOARD_HTML_PATH =
  DASHBOARD_CANDIDATES.find((p) => existsSync(p)) ?? DASHBOARD_CANDIDATES[0];
const DASHBOARD_HTML = existsSync(DASHBOARD_HTML_PATH)
  ? readFileSync(DASHBOARD_HTML_PATH, "utf-8")
  : "<html><body><h1>Dashboard not found</h1><p>Expected at: " +
    DASHBOARD_CANDIDATES.join(" or ") +
    "</p></body></html>";
  ...
  fetch(req, server) {
      ...
      return Response.json(
        all.map((p) => ({
          ...
          stage_summary: {
            ...
            completed: p.state.stages.filter((s) => s.status === "completed")
              .length,
    ...
    if (url.pathname === "/api/v1/pipelines" && req.method === "POST") {
      return req
        .json()
        .then(
          (body: { id?: string; template: string; change_name?: string }) => {
            if (!body.template) {
              return Response.json(
            ...
            if (existing) {
              return Response.json({
                ...
                id: existing.id,
                template: tpl.name,
                change_name: existing.state.change_name,
                resumed: true,
              });
            }
            ...
            appendAudit(
              "pipeline_created",
              "",
              `${inst.id} (change: ${body.change_name ?? ""})`,
              inst.id,
            );
            return Response.json({
              status: "ok",
              id: inst.id,
              template: tpl.name,
              change_name: inst.state.change_name,
              resumed: false,
            });
          },
        )
    ...
    if (url.pathname === "/api/v1/pipeline/state" && req.method === "GET") {
      ...
      if (!inst)
        return Response.json({ error: "no pipeline found" }, { status: 404 });
    ...
    if (url.pathname === "/api/v1/pipeline/templates" && req.method === "GET") {
      ...
      for (const pDir of getAllPipelineDirs()) {
        const projectName =
          [...registeredPipelineDirs.entries()].find(
            ([, v]) => v === pDir,
          )?.[0] ?? "local";
    ...
    // 注册项目模板目录
    if (
      url.pathname === "/api/v1/templates/register" &&
      req.method === "POST"
    ) {
      return req
        .json()
        .then((body: { project: string; dir: string }) => {
          if (!body.project || !body.dir) {
            return Response.json(
              { error: "project and dir are required" },
              { status: 400 },
            );
          }
          ...
          appendAudit(
            ...
            "",
            `${body.project} → ${absDir} (${templates.length} templates)`,
            "",
          );
          return Response.json({
            status: "ok",
            ...
            templates: templates.map((f) => {
              try {
                return loadTemplate(join(absDir, f)).name;
              } catch {
                return f;
              }
            }),
    ...
    // 注销项目模板目录
    if (
      url.pathname === "/api/v1/templates/unregister" &&
      req.method === "POST"
    ) {
      return req
        .json()
        .then((body: { project: string }) => {
          if (!body.project) {
            return Response.json(
    ...
    // 列出已注册的模板目录
    if (
      url.pathname === "/api/v1/templates/registered" &&
      req.method === "GET"
    ) {
    ...
    if (url.pathname === "/api/v1/pipeline/change" && req.method === "POST") {
      return req
        .json()
        .then((body: { name: string; pipeline?: string }) => {
          ...
          if (!inst.state.started_at)
            inst.state.started_at = new Date().toISOString();
          savePipelineState(inst);
          broadcast("change_updated", {
            pipeline_id: inst.id,
            name: inst.state.change_name,
          });
    ...
    if (url.pathname === "/api/v1/pipeline/stage" && req.method === "POST") {
      return req
        .json()
        .then((body: { stage: string; status: string; pipeline?: string }) => {
          const id = body.pipeline ?? url.searchParams.get("pipeline");
          let inst: PipelineInstance | null = null;
          if (id) inst = loadPipeline(id);
          if (!inst) inst = getActivePipeline(url);
          ...
          if (body.status === "completed" || body.status === "failed") {
            ...
            if (s.started_at)
              s.duration_ms =
                new Date(now).getTime() - new Date(s.started_at).getTime();
          }
          savePipelineState(inst);
          broadcast("stage_updated", {
            pipeline_id: inst.id,
            stage: body.stage,
            status: body.status,
          });
    ...
    if (url.pathname === "/api/v1/pipeline/gate" && req.method === "POST") {
      return req
        .json()
        .then((body: { gate: string; result: string; pipeline?: string }) => {
          const id = body.pipeline ?? url.searchParams.get("pipeline");
          let inst: PipelineInstance | null = null;
          if (id) inst = loadPipeline(id);
          if (!inst) inst = getActivePipeline(url);
...
console.log(
  `Registered dirs: ${registeredPipelineDirs.size > 0 ? [...registeredPipelineDirs.entries()].map(([k, v]) => `${k}→${v}`).join(", ") : "(none)"}`,
);
console.log(
  `Pipelines:       ${
    listPipelines()
